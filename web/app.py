...
def login():
    ...
    add_log(
        f"Preparing login request to {api_client._login_url} for user {username}"
    )
    try:
        ...
        if not inverters:
            add_log(
                "Login succeeded but no inverters were found for this account"
            )
            return (
...
def voltage():
    ...
    try:
        add_log(
            f"Sending request to {api_client._inverter_battery_url} for voltage"
        )
